...
class EncoderBlock(nnx.Module):
    ...
    def __call__(
        ...
        h = x + self.attention(
            self.attention_norm(x), start_pos=0, sincos=sincos, full_causal_mask=None
        )
...
def main():
    bs, seq_len = 2, 8
    rngs = nnx.Rngs(0)
    args = ModelArgs(max_batch_size=bs, max_seq_len=512)
    self = EncoderBlock(args, rngs)
    x = jax.random.normal(rngs.value(), (bs, seq_len, args.dim))
